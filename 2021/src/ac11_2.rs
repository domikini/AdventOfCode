...
pub fn ac11_2() -> Result<(), Error>{
    ...
    while true {
        ...
        if(have_all_octopuses_fired(&result.map)){
            break;
        }
        result.map = reset_all_flashed_fired_to_zero(&result.map);
    }
    println!("Svar: 11_2: {}", iteration);
    Ok(())
}
...
fn have_all_octopuses_fired(map:&Array2D<usize>) -> bool {
    let mut map_vec: Vec<usize> = map.as_row_major();
    let mut have_all_octopuses_fired:bool = false;
    map_vec = map_vec.into_iter().filter(|e|e >= &(9999 as usize)).collect();
    if(map_vec.len() == 100){
        have_all_octopuses_fired = true;
    }
    return have_all_octopuses_fired;
}