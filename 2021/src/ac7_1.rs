...
pub fn ac7_1() -> Result<(), Error>{
    ...
    println!("Svar 7_1: {:?}", posstats.iter().min_by_key(|p|p.fuel_consumption).unwrap().fuel_consumption);
    Ok(())
}
...
fn calculate_fuel_consumption_to_move(crab_list:&Vec<Crab>, new_hoz_pos:&usize) -> Vec<Crab>{
    ...
    for mut c in &mut crab_list{
        if (c.horizontal_pos > *new_hoz_pos) {
            c.fuel_to_new_hoz_pos = c.horizontal_pos - new_hoz_pos;
        }
        else {
            c.fuel_to_new_hoz_pos = new_hoz_pos - c.horizontal_pos;
        }
    }
    return crab_list;
}
